...
export const PetFeed = () => {
  ...
  return <div className="d-flex flex-wrap justify-content-center">
    {animals.map(pet => {
      return (
        <PetCard key={pet.id} description={lorem} weight={pet.weight} name={capitalize(pet.name)} age={pet.age} city={pet.city} state={pet.state} image={`${process.env.REACT_APP_BACKEND_URL}/pets/${pet.id}/image`} />
      )
    })}
  </div>
}